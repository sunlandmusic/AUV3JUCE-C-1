...
TitleComponent::TitleComponent() : xlButton("XL")
{
    ...
    titleLabel.setFont(juce::Font(titleFontSize, juce::Font::FontStyleFlags::light)); // fontWeight: '300' is light
    ...
    xlButton.setConnectedEdges(juce::TextButton::ConnectedEdgeFlags::dontConnectToAnything); // For custom rounding
    ...
    juce::Font xlFont(xlButtonFontSize, juce::Font::FontStyleFlags::normal); // fontWeight: '400' is normal
...
float TitleComponent::getOriginalUnrotatedWidth() const
{
    ...
    juce::Font titleFont(titleFontSize, juce::Font::FontStyleFlags::light);
    float titleTextWidth = titleFont.getStringWidthFloat(titleLabel.getText());
...
void TitleComponent::resized()
{
    ...
    xlButton.setBounds(localBounds.getWidth() - xlButtonWidth,
                       (localBounds.getHeight() - xlButtonHeight) / 2, // Centered vertically
                       xlButtonWidth,
                       xlButtonHeight);

    // Title Label is on the left (before rotation: bottom)
    juce::Rectangle<int> titleLabelBounds;
    titleLabelBounds.setWidth(localBounds.getWidth() - xlButtonWidth - internalPadding);
